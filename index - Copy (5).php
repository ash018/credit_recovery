...
<script type="text/javascript">
	window.onload = function () {
		...
		<?php
			...
			echo $query = "SELECT T1.PartName, (SUM(T2.actualpaidamount)/1000000) AS CollectionAmt, (SUM(T3.ProjectionAmount)/1000000) AS ProjectionAmt FROM partinformation T1
	...
	var chart = new CanvasJS.Chart("chartcontainerODComparison", {
		...
		<?php
			...
			$string = ''; 
			$threemonth = date('Y-m-d',strtotime("-90 days"));  //New column graph for displaying projection vs collection
			$query = "SELECT (SUM(T1.LMOD)/1000000) AS LMOD, (SUM(T1.SPLYOD)/1000000) AS SPLYOD, (SUM(T2.overdueamount)/1000000) AS OD, T1.Part
						FROM OverDueInformation T1
						INNER JOIN creditrecovery T2 ON T1.Code = T2.CustomerCode						
						GROUP BY T1.Part
						UNION
						SELECT (SUM(T1.LMOD)/1000000) AS LMOD, (SUM(T1.SPLYOD)/1000000) AS SPLYOD, (SUM(T2.overdueamount)/1000000) AS OD, 'National' AS Part
						FROM OverDueInformation T1
						INNER JOIN creditrecovery T2 ON T1.Code = T2.CustomerCode						
						";
			$result = mysqli_query($link, $query);		
