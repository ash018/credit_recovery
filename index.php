...
<script type="text/javascript">
	...
	window.onload = function () {
		...
		<?php
			$count = 0;
			$string = ''; 
			$query = "EXEC ups_daily_projection_vs_collection";	
	...
	var chart = new CanvasJS.Chart("chartContainer", {
		...
		<?php
			$count = 0;
			$string = ''; 			
			$query = "Exec ups_monthly_projection_vs_collection '$Date1', '$Date2'";	
			$result = odbc_exec($connection,$query);	
			$x = 1;	
			while($row=odbc_fetch_array($result)) {
				$count++;
	...
	var chart = new CanvasJS.Chart("perfileod", {
		...
		<?php
			$count = 0;
			$string = ''; 			
			$query = "Exec usp_perfile_OD";	
			$result = odbc_exec($connection,$query);	
			$x = 1;	
			while($row=odbc_fetch_array($result)) {
				$count++;
				$string .= " { y: ".$row['tCMOD'].", label: '".$row['Part']."'}, ";
	...
	<div class="page-content-wrapper">
		<div class="page-content">
			<!-- BEGIN DASHBOARD GRAPH -->	
				<div style="margin-top: 1px;">
					<div class="col-md-12" style="position: fixed; bottom: 0; z-index:999999; padding: 10px; background: black; color: white;"><b>
						<marquee>Today's Collection:&nbsp;
						<?php
							$mq_query = "SELECT territoryname,SUM(actualpaidamount) AS collectionamount FROM creditrecovery Where visitdate=convert(varchar(10), getdate(), 120) GROUP BY territoryname";
							$mq_result = odbc_exec($connection,$mq_query);							
							
							$mq_query_t = "SELECT SUM(actualpaidamount) AS Tcollectionamount FROM creditrecovery Where visitdate=convert(varchar(10), getdate(), 120)";
							$mq_result_t = odbc_exec($connection,$mq_query_t);			
							$mq_row_t = odbc_fetch_array($mq_result_t);
							echo "Total:".floor($mq_row_t['Tcollectionamount'])."&nbsp;,&nbsp;";
			...
			<div class="row">
				<div class="col-md-4 col-sm-6">
					<div class="portlet light ">						
						<a href="piechart3.php"><div class="portlet-body" id="piechart3" style="height: 250px; width: 100%">
				...
				<div class="col-md-4 col-sm-6">
					<div class="portlet light ">						
						<a href="piechart2.php"><div class="portlet-body" id="piechart2" style="height: 250px; width: 100%">
			...
			<div class="row">
				...
				<?php
					$query_1 = "SELECT COUNT(CMNoInstOD) AS T1 FROM overduedetail
											where (CMNoInstOD>0.0 AND CMNoInstOD<=2)";
					...
					$query_2 = "SELECT COUNT(CMNoInstOD) AS T2 FROM overduedetail
								where (CMNoInstOD>=2 AND CMNoInstOD<=4)";
					...
					$query_3 = "SELECT COUNT(CMNoInstOD) AS T3 FROM overduedetail
								where (CMNoInstOD>=4 AND CMNoInstOD<=5)";
				...
				<div class="col-md-4 col-sm-6">
					<div class="portlet light ">						
						<div class="portlet-body" style="height: 250px; width: 100%">
							<p>
								<?php
									$query_a = "SELECT SUM(actualpaidamount) AS Aamt FROM creditrecovery WHERE visitdate between '$SDate' AND '$EDate'";									
									$result_a = odbc_exec($connection,$query_a);
									$row_a = odbc_fetch_array($result_a)
								?>
								Recovery-(<?php echo $row_a['Aamt']."/=";  ?>)
							</p>
							<p>
								<?php
									$query_b = "Select sum(InstSize) AS SumOfInst from overduedetail";									
									$result_b = odbc_exec($connection,$query_b);
									$row_b = odbc_fetch_array($result_b)
								?>
								Credit-(<?php echo $row_b['SumOfInst']."/=";  ?>)
							</p>
							<p>
								<?php
									$query_c = "SELECT COUNT(T1.AssessmentCode) AS tassmnt FROM TmpAssessmentAnswer T1
												INNER JOIN AssessCustomer T2 ON T1.AssessmentCode = T2.AssessmentCode
												WHERE AssessmentDate BETWEEN '$SDate' AND '$EDate'";									
									$result_c = odbc_exec($connection,$query_c);
									$row_c = odbc_fetch_array($result_c)
								?>
								Assessment-(<?php echo $row_c['tassmnt'];  ?>)
							</p>
							<p>Reminder-(12)</p>
						</div>
					</div>
...
<div class="page-footer">
	<div class="page-footer-inner">
		 2017 © ACI Ltd.<a href="#" title="ACI Ltd" target="_blank">Developed By kallul</a>
	</div>
...
<?php
	$query_chart0_green = "SELECT COUNT(CMNoInstOD) AS T1 FROM overduedetail
							where Part='South' AND (CMNoInstOD>=0 AND CMNoInstOD<=2)";
	...
	$query_chart0_red = "SELECT COUNT(CMNoInstOD) AS T2 FROM overduedetail
							where Part='South' AND (CMNoInstOD>=2 AND CMNoInstOD<=4)";
	...
	$query_chart0_yellow = "SELECT COUNT(CMNoInstOD) AS T3 FROM overduedetail
						where Part='South' AND (CMNoInstOD>=4 AND CMNoInstOD<=5)";
...
<?php
	$query_chart1_green = "SELECT COUNT(ServiceSatisfaction) as green FROM projection 
							WHERE (customerBehavior<>'' AND ServiceSatisfaction<>'') AND
							(ServiceSatisfaction = 'Green') AND (CollectionDate BETWEEN '$SDate' AND '$EDate')";
